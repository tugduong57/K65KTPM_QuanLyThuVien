...
def edit_book(root, Root):
    ...
    def sua_sach() :
        #  labels_left = ['Mã sách', 'Tên sách', 'Thể loại', 'Tác giả']
        #  labels_right = ['Nhà xuất bản', 'Ngày sản xuất', 'Số lượng', 'Giá']
        connectSQL.update_sach(entries_left[0].get(), entries_left[1].get(), entries_left[2].get(), entries_right[1].get(), entries_right[0].get(), entries_left[3].get(), entries_right[3].get(), entries_right[2].get())
